...
/**
 * Takes the goods on an invoice into stock, when it reaches `proceed`.
 *
 ...
 *
 * Idempotent: an invoice already in the ledger is left exactly as it is, so
 * callers can ask for the goods to be in stock without first checking whether
 * they already are. `RevertPurchaseAction` is the way back out.
 */
final class ReceivePurchaseAction
{
    ...
    public function handle(Purchase $purchase): Purchase
    {
        ...
        if ($purchase->lines->isEmpty()) {
            throw PurchaseLedgerException::hasNoLines($purchase);
        }
        ...
        return DB::transaction(function () use ($purchase, $landed): Purchase {
            ...
            $purchase->forceFill(['committed_at' => now()])->save();
