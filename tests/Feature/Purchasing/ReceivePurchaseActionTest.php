...
beforeEach(function () {
    $this->receive = app(ReceivePurchaseAction::class);
 ...
 */
function orderedPurchase(array $lines, array $costs = [], string $on = '2026-01-15'): Purchase
{
...
it('writes one ledger movement per line, traceable back to it', function () {
    ...
    $purchase = orderedPurchase(lines: [
...
it('stamps when the goods reached the ledger', function () {
    $purchase = orderedPurchase(lines: [
        ['product' => Product::factory()->create(), 'quantity' => 1, 'unit_cost' => '1.00'],
    ]);

    $received = $this->receive->handle($purchase);

    expect($received->isCommitted())->toBeTrue()
        ->and($received->committed_at)->not->toBeNull();
});
...
it('takes the same invoice in only once', function () {
    $purchase = orderedPurchase(lines: [
    ...
    // Idempotent rather than an error: callers ask for the goods to be in
    // stock, and they already are.
    $this->receive->handle($purchase->refresh());

    expect(StockMovement::query()->count())->toBe(1);
});

it('refuses to take in an empty invoice', function () {
    $purchase = Purchase::factory()->create(['status' => PurchaseStatus::Ordered]);

    expect(fn () => $this->receive->handle($purchase))
        ->toThrow(PurchaseLedgerException::class);
...
it('lets purchased stock be sold at the landed cost, oldest first', function () {
    ...
    $january = orderedPurchase(
        lines: [['product' => $product, 'quantity' => 10, 'unit_cost' => '5.00']],
        on: '2026-01-01',
    );
    $february = orderedPurchase(
        lines: [['product' => $product, 'quantity' => 10, 'unit_cost' => '7.00']],
        on: '2026-02-01',
    );

    $this->receive->handle($january);
    $this->receive->handle($february);
